...
// 
int
u_mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm) {
  uint64 a, last;
  pte_t *pte;
...
void
vmprintprocess(pagetable_t pagetable, int level) {
  
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if((pte & PTE_V)){  // valid
      for(int times = 0; times<level; times++) {
        printf(".. ");
      }
      printf("%d: pte %p pa %p\n", i, pte, PTE2PA(pte));  // to physical addr
      
      // see vm.c:285, go to next level vs. leaf page
      if ((pte & (PTE_R|PTE_W|PTE_X)) == 0) {
        uint64 child = PTE2PA(pte);
        vmprintprocess((pagetable_t)child, level+1);
      }
    }
  }
}
...
// copy user page to kernel page
int u2kcopy(pagetable_t user_pg, pagetable_t k_pg, uint64 begin, uint64 end) {
  pte_t* pte;
  uint64 pa, i;
  uint64 flags;
  ...
  for(i = begin; i<end; i+=PGSIZE) {
    ...
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte) & (~PTE_U); // mask off PTE_U so that kernel can access
    if (u_mappages(k_pg, i, PGSIZE, pa, flags) != 0) {
      uvmunmap(k_pg, 0, i/PGSIZE, 1); // release
      return -1;
    }
  }
  return 0;
